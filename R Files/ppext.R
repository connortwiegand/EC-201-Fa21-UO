...
nomarks <- ggplot() + 
  ...
  # Bounds and Scale
  scale_x_continuous(breaks=seq(0, x_lim, 100), expand=c(0,0), limits=c(0, x_lim)) +
  scale_y_continuous(breaks=seq(0, y_lim, 200), expand=c(0,0), limits=c(0, y_lim)) +
  ...
  #msc/MSC Label:
  annotate("text", x=19250, y=5200, label="MSC", 
           fontface = "bold", color = red_pink, size = 4)
nomarks
...
base <- nomarks + 
  ...
  #MEB/MEC grey lines
  annotate("segment", x=15000, xend=15000, y=demand(15000)+150, yend=msc(15000)-150,
...
cpext_eb <- base +
  ...
  geom_ribbon(aes(x=0:eq$x, ymin = msc(0:eq$x), ymax = demand(0:eq$x)), alpha = 0.2, show.legend = F,
              fill = green)
cpext_eb
...
cpext_dwl <- base + 
  ...
  geom_ribbon(aes(x=eq$x:eq2$x, ymin = supply(eq$x:eq2$x), ymax = msc(eq$x:eq2$x)), alpha = 0.2, show.legend = F,
              fill = "purple")
cpext_dwl
...
nomarks <- ggplot() + 
  ...
  # Market Modification (Shift, Control, Distortion)
  geom_line(aes(x=q_range, y=msc(q_range)), size = 1, color = red_pink) +
  #S/D Labels:
  annotate("text", x=17000, y=500, label="D~MPB", 
           fontface = "bold", color = red_pink, size = 4) + 
  annotate("text", x=19500, y=11000, label="S", 
           fontface = "bold", color = purple, size = 4) +
  #msc/MSC Label:
  annotate("text", x=17000, y=4300, label="bold(D[t]==msc)", 
...
csps <- base +
  ...
  geom_ribbon(aes(x=0:eq2$x, ymin=eq2$y, ymax = msc(0:eq2$x)), alpha = 0.2, show.legend = F, 
              fill = "red") +
  #PS
  annotate("text", x=2000, y=5500, label="PS", color = purple) +
...
cpext_eb <- base +
  ...
  geom_ribbon(aes(x=0:eq2$x, ymin = msc(0:eq2$x), ymax = demand(0:eq2$x)), alpha = 0.2, show.legend = F,
              fill = green)
cpext_eb
